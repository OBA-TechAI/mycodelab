...
"""
# ============================================================
# 13. FT-TRANSFORMER - PURE PYTORCH VERSION
# ============================================================
...
class FTTabularDataset(Dataset):
    ...
    def __init__(
        ...
        x_num = frame[numeric_cols].to_numpy(dtype=np.float32)
        ...
        self.x_cat = torch.tensor(
            frame[binary_cols].to_numpy(dtype=np.int64),
            dtype=torch.long,
        )
...
class FTTransformerNative(nn.Module):
    ...
    def __init__(
        ...
        # Each binary feature gets its own embedding
    ...
    def forward(
        ...
        # Convert each numeric value into a token
        numeric_tokens = (
            x_num.unsqueeze(-1)
        ...
        # Convert binary variables into tokens
        binary_tokens = []
...
def train_fttransformer(
    ...
    for epoch in range(
        ...
        y_val, p_val = predict_fttransformer(
            model,
            val_loader,
        )
        ...
        print(
            f"FT-Transformer epoch {epoch}: "
            f"loss="
            f"{running_loss/max(len(train_loader),1):.4f} "
            f"val_AUROC={val_auc:.4f}"
        )
    ...
    # Final test
    y_test, p_test = predict_fttransformer(
        model,
        test_loader,
    )
    ...
    torch.save(
        ...
        output_dir
        / "fttransformer_longitudinal_dn.pt",
    )
...
def train_mlp(
    ...
    X_val_num = scaler.transform(
        val_frame[NUMERIC_COLS]
    )

    X_test_num = scaler.transform(
        test_frame[NUMERIC_COLS]
    )

    # Binary variables remain 0/1
    X_train_bin = train_frame[
        BINARY_COLS
    ...
    y_test_eval, p_test = predict_mlp(
        model,
        test_loader,
    )
...
def train_lstm(
    ...
    # Avoid HPC cuDNN version mismatch for LSTM
...
def train_gru(
    ...
    # Avoid HPC cuDNN version mismatch
    torch.backends.cudnn.enabled = False
...
# Logistic Regression follows test_tab order.
logistic_test_ids = (
    test_tab[ID_COL]
    .reset_index(drop=True)
)
...
# ============================================================
# SAVE / UPDATE EXPANDED MASTER RESULTS
# ============================================================

results_file = (
    DATA_DIR
    / "dn_expanded_MASTER_results.csv"
)
...
if results_file.exists():
    ...
    # If a model is rerun, keep its newest result.
    results_df = (
        results_df
        .drop_duplicates(
            subset=["Model"],
...
# ============================================================
# SAVE / UPDATE EXPANDED MASTER PREDICTIONS
# ============================================================

predictions_file = (
    DATA_DIR
    / "dn_expanded_MASTER_predictions.csv"
)
...
if predictions_file.exists():
    ...
    # Remove old version if the same model is rerun.
    old_predictions = (
        old_predictions.drop(
            columns=new_probability_cols,
            errors="ignore",
        )
    )
...
"""
    results_file = (
        DATA_DIR
	/ "dn_four_model_MASTER_results.csv"
...
predictions_file = (
   ...
   # / "dn_longitudinal_test_predictions.csv"
    / "dn_four_model_MASTER_predictions.csv"
)
