...
for sid, patient_df in seq.groupby(
    ...
    for visit_number, visit_df in patient_df.groupby(
        ...
        # Diagnosis tokens
        for code in visit_df["icd_token"]:
    ...
    if len(tokens) > MAX_LEN:
        ...
        ages = ages[-MAX_LEN:]
        visit_types = visit_types[-MAX_LEN:]
        gaps = gaps[-MAX_LEN:]
...
BEST_MODEL = (
    "timeaware_cehrbert_365d_best.pt"
)
...
results = {
    ...
    "Model":
        "Time-Aware CEHR-BERT-style 365d",
...
print("=" * 65)
print(
    "LEAKAGE-SAFE TIME-AWARE CEHR-BERT-STYLE CONTROL RESULTS"
)
print("=" * 65)
...
pd.DataFrame(
    ...
    "timeaware_cehrbert_365d_results.csv",
    index=False
)
...
pd.DataFrame({
    ...
    "timeaware_cehrbert_365d_predictions.csv",
    index=False
)

print(
    "\nSaved: timeaware_cehrbert_365d_results.csv"
)

print(
    "Saved: timeaware_cehrbert_365d_predictions.csv"
)
