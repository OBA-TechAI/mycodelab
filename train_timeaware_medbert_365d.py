...
# ============================================================
# 4. VOCABULARY — TRAINING PATIENTS ONLY
# ============================================================
...
# ============================================================
# 6. BUILD PATIENT DIAGNOSIS SEQUENCES
# ============================================================
...
for sid, g in seq.groupby("subject_id"):
    ...
    # Keep most recent diagnosis history
    if len(g) > MAX_LEN:
        truncated += 1
        g = g.iloc[-MAX_LEN:]

    codes = [
        code_to_id.get(code, 1)
        for code in g["icd_token"]
    ]
...
class MedBERTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        codes = list(p["codes"])
        visits = list(p["visits"])
        ranks = list(p["ranks"])
        gaps = list(p["gaps"])
        recency = list(p["recency"])
...
class MedBERTStyleClassifier(nn.Module):
    ...
    def forward(
        ...
        x = (
            ...
            + self.visit_embedding(visits)
            + self.rank_embedding(ranks)
            + self.time_gap_embedding(gaps)
            + self.time_to_cutoff_embedding(recency)
...
model = MedBERTStyleClassifier().to(DEVICE)
    ...
    with torch.no_grad():
        ...
        for batch in loader:
            ...
            logits = model(
                ...
                batch["visits"].to(DEVICE),
                batch["ranks"].to(DEVICE),
...
BEST_MODEL = "timeaware_medbert_365d_best.pt"
...
results = {
    ...
    "Model":
        "Time-Aware Med-BERT-style 365d",
...
print("\n")
print("=" * 60)
print("FINAL LEAKAGE-SAFE MED-BERT-STYLE CONTROL RESULTS")
print("=" * 60)
...
pd.DataFrame(
    ...
    "timeawaremedbert_365d_results.csv",
    index=False
)
...
pd.DataFrame({
    ...
    "timeaware_medbert_365d_predictions.csv",
    index=False
)

print(
    "\nSaved: timeaware_medbert_365d_results.csv"
)

print(
    "Saved: timeaware_medbert_365d_predictions.csv"
)
