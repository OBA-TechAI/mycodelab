...
BEST_DIR = "llama32_3b_lora_timeaware_365d_adapter"

RESULT_FILE = "llama32_3b_lora_timeaware_365d_results.csv"
PRED_FILE = "llama32_3b_lora_timeaware_365d_predictions.csv"
...
# ============================================================
# 3. BUILD TIME-AWARE DIAGNOSIS TEXT
# ============================================================
...
for sid, patient_df in seq.groupby("subject_id"):
    ...
    for visit_number, visit_df in patient_df.groupby(
        ...
        visit_df = visit_df.sort_values("code_rank")
        ...
        # Temporal values are shared by diagnoses in the visit.
        time_gap = float(
            visit_df["time_gap_days"].iloc[0]
        )

        time_to_cutoff = float(
            visit_df["time_to_cutoff_days"].iloc[0]
        )
        ...
        visit_text = (
            ...
            + ", "
            + str(round(time_to_cutoff, 1))
            + " days before prediction cutoff, "
            + str(round(time_gap, 1))
    ...
    patient_label[sid] = int(
        patient_df["dn_within_365d"].iloc[0]
    )
...
if SMALL_TEST:
    ...
    def select_small_subset(ids, n):
        ids = np.asarray(ids)
        labels = np.asarray(
            [patient_label[int(sid)] for sid in ids]
        )

        positive_ids = ids[labels == 1]
        negative_ids = ids[labels == 0]

        n_positive = max(
            1,
            round(n * len(positive_ids) / len(ids))
        )
        ...
        selected = np.concatenate([
            rng.choice(
                positive_ids,
                size=min(n_positive, len(positive_ids)),
                replace=False
            ),
            rng.choice(
                negative_ids,
                size=min(n_negative, len(negative_ids)),
                replace=False
            )
        ])
    ...
    train_ids = select_small_subset(
        train_ids, SMALL_TRAIN_N
    )

    val_ids = select_small_subset(
        val_ids, SMALL_VAL_N
    )

    test_ids = select_small_subset(
        test_ids, SMALL_TEST_N
    )

    print("\nSMALL TEST MODE ENABLED")
...
results = {
    "Model":
        "Llama 3.2 3B Instruct LoRA Time-Aware Diagnosis-Text 365d",
